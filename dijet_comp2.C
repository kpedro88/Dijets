...
void dijet_comp2(bool print=false, string psuff="png", string pdir="plots", TFile* file=NULL){
	//open histogram file
				...
				for(int iet = 0; iet < 1; ++iet){
					...
					for(int ipt = 0; ipt < bins.pt.size()-1; ++ipt){
						...
						for(int ial = 0; ial < bins.alpha.size()-1; ++ial){
							...
							//option maps for fit(s), add to dist
							vector<KNamed*> fitLines;
							OptionMap* opt_f = new OptionMap();
							opt_f->Set<Color_t>("color",kBlue);
							opt_f->Set<int>("line",2);
							opt_f->Set<bool>("subtract_gaus",false); //test
							opt_f->Set<bool>("propagate_sigma_bin",false); //test
